...
class CachedMapDistanceFinder:
    ...
    def get_map_cost_between(self, src_junction: Junction, tgt_junction: Junction) -> Optional[Cost]:
        ...
        """
        if self._is_in_cache((src_junction.index, tgt_junction.index)) is True:
            return self._get_from_cache((src_junction.index, tgt_junction.index))
        else:
            search_result = self.map_problem_solver.solve_problem(MapProblem(self.streets_map, src_junction.index, tgt_junction.index))
            if search_result.is_solution_found is False:
                road_len = None
            else:
                road_len = search_result.solution_g_cost
            self._insert_to_cache((src_junction.index, tgt_junction.index), road_len)
            return road_len
